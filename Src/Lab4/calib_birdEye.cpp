...
void Calibration(const char* argv[])
{
...
void BirdEye(const char* argv[])
{
    ...
    if (!fs.isOpened() || intrinsic.empty() || distortion.empty()) {
        cout << "Error: Couldn't load intrinsic parameters from " << "intrinsics.xml"
            << endl;
        exit(-1);
    }
    fs.release();
...
//Input:
/*<1board_width> <2board_height> <3number_of_boards>
<4ms_delay_framee_capture> <5image_scaling_factor> 
<6path_to_calibration_images><7path/chessboard_image>\n\n"
*/
